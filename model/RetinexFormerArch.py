...
class Illumination_Estimator(tfkl.Layer):
    def __init__(self, n_fea_middle, n_fea_out=3):
            ...
            self.depth_conv = tfkl.DepthwiseConv2D(kernel_size=5, padding='same', use_bias=True)
    ...
    def __init__(
        ...
        heads=8,
            ...
            self.pos_emb = Sequential(
                [
                    tfkl.DepthwiseConv2D(kernel_size=3, strides=1, padding='same', use_bias=True, activation="gelu"),
...
class IGAB(tfkl.Layer):
    ...
    def call(self, x, illu_fea):
            # x = tf.transpose(x, [0, 2, 3, 1])   # change made here
            for attn, ff in self.blocks:
                x = attn(x, illu_fea_trans=illu_fea) + x  # change made here
                x = ff(x) + x
            out = x
            # out = tf.transpose(x, [0, 3, 1, 2])    # change made here
            return out


class Corruption_Restorer(tfkl.Layer):
    def __init__(self, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4]):
...
class RetinexFormer_Single_Stage(tfkl.Layer):
    def __init__(self, out_channels=3, n_feat=40, level=2, num_blocks=[1, 1, 1]):
...
class RetinexFormer(tfkm.Model):
    def __init__(self, out_channels=3, n_feat=40, stage=3, num_blocks=[1, 1, 1]):
